...
Without any  extra options, the mounted  CIFS shares
...
> Options   to   mount.cifs   are   specified   as   a
> comma-separated list of key=value pairs.

The `man  mount.cifs` is not  long (see at  the very
end because this is the NixOS-specific command), and
`mount.cifs` provides the `uid` option:
...
>   on  FILE  AND DIRECTORY OWNERSHIP AND PERMISSIONS below for more informa‐
>   tion.

It  does  accept a  string,  but  numeric `uid`  are
always a  safe bet to  use instead. To look  mine up
I  simply  used the  `-n`  switch  for `ls`  on  any
...
> ```
> and  on  how  to  capture `man`  pages  without  the
> annoying control characters, read this:
> [Save a Unix manpage as plain text](https://electrictoolbox.com/save-manpage-plain-text/)
